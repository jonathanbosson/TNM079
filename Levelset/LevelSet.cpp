...
float LevelSet::Diff2Xpm(int i, int j, int k) const
{
	/*
	if (F > 0)
	std::max( pow(std::max(DiffXm(i,j,k), 0.0f),2) , pow(std::min(DiffXp(i,j,k), 0.0f),2) );
	else if (F < 0)
	std::max( pow(std::min(DiffXm(i,j,k), 0.0f),2) , pow(std::max(DiffXp(i,j,k), 0.0f),2) );
	*/
