...
std::vector<unsigned int> HalfEdgeMesh::FindNeighborVertices(unsigned int vertexIndex) const
{
	...
	HalfEdge eTemp = e(e1.prev);
	do 
	{
		oneRing.push_back(eTemp.vert);
		eTemp = e(e(eTemp.pair).prev);
	} while (eTemp.pair != v1.edge);
	oneRing.push_back(eTemp.vert);
...
std::vector<unsigned int> HalfEdgeMesh::FindNeighborFaces(unsigned int vertexIndex) const
{
	...
	HalfEdge eTemp = e(e(e1.prev).pair);
	do 
	{
		foundFaces.push_back(eTemp.face);
		eTemp = e(e(eTemp.prev).pair);
	} while (eTemp.next != e1.next);
...
float HalfEdgeMesh::Area() const
{
  ...
  for (unsigned int i = 0; i < mFaces.size(); i++) {
	...
	Vector3<float> vec1 = v(e(e1.prev).vert).pos - v(e1.vert).pos;
	Vector3<float> vec2 = v(e(e1.next).vert).pos - v(e1.vert).pos;
	
	area += Cross(vec1, vec2).Length() / 2.0;
  }
...
float HalfEdgeMesh::Volume() const
{
  ...
  for(int i = 0; i < mFaces.size(); i++) {
		HalfEdge e1 = e(f(i).edge);
		Vector3<float> c = (v(e1.vert).pos + v(e(e1.next).vert).pos + v(e(e1.prev).vert).pos) / 3.0;
